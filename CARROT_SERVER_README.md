...
The reset route is intentionally disabled until a verified SMS provider is connected. The current API is intended for a trusted local network; production use still requires HTTPS, a real database, stronger user management, audit logging, and a real SMS provider.
